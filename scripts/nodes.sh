...
运行环境:
  ...
  前置步骤: ./scripts/local.sh bootstrap --profile FILE 会自动调用 install manager。
...
前置条件:
  .venv 已存在; 否则先运行 ./scripts/local.sh bootstrap --profile FILE。
  uv 可用。
...
require_venv() {
  [[ -x "$PYTHON_BIN" ]] || die "$PYTHON_BIN not found; run ./scripts/local.sh bootstrap --profile FILE first" 2
}
