...
`remote.sh` 也按同样思路拆成薄入口 + Shell 子模块：

```text
scripts/remote.sh           只 source 模块并按一级命令 dispatch
scripts/remote/usage.sh     remote.sh 顶层和子命令 help
scripts/remote/core.sh      配置默认值、参数校验、SSH 命令拼装公共函数
scripts/remote/models.sh    remote.sh models 子命令、远端模型下载日志和 upload
scripts/remote/actions.sh   sync/bootstrap/lifecycle/status/logs/ready/tunnel/gpu 动作
```

`verify.sh` 是薄入口，复杂 smoke 放到 Python 子模块：

```text
scripts/verify.sh                  只定位仓库根目录并调用 Python CLI
scripts/verify/cli.py              verify.sh 的命令分发
scripts/verify/syntax.py           shell/python 语法与 shellcheck 检查
scripts/verify/help.py             入口和子命令 help smoke
scripts/verify/read_only.py        不联网、不启动服务的只读 smoke
scripts/verify/models_contract.py  配置、catalog schema、模型下载/状态合同 smoke
scripts/verify/remote_contract.py  remote.sh 远程命令拼装和 upload 合同 smoke
scripts/verify/common.py           验证 helper、临时目录、subprocess 封装
```
